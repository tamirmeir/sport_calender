...
// Load tournament master data for knockout rules
const TOURNAMENTS_MASTER_FILE = path.resolve(__dirname, '../data/world_tournaments_master.json');
let tournamentsData = null;

function getTournamentKnockoutRules(leagueId) {
    if (!tournamentsData) {
        try {
            tournamentsData = JSON.parse(fs.readFileSync(TOURNAMENTS_MASTER_FILE, 'utf-8'));
        } catch (err) {
            console.error('[API] Failed to load tournaments master data:', err.message);
            return null;
        }
    }
    
    const tournament = tournamentsData.tournaments?.[String(leagueId)];
    return tournament?.knockout || null;
}
            ...
            if (fs.existsSync(ACTIVE_LEAGUES_FILE)) {
                ...
                if (fromCache.length > 0) {
                    ...
                    const lightweight = fromCache.map(l => ({
                        ...
                        ui_label: l.verify_status === 'active' ? "⚽ Active" : "🏖️ Vacation"
                    }));
        ...
        try {
            ...
            if (result.isFinished && (details.isAnnualCup || details.isTournament)) {
                ...
                if (winner) {
                    ...
                    result.statusLabel = `🏆 ${winner.name}`;
                }
            }
            ...
            if (result.isFinished && details.isLeague) {
                ...
                if (standings?.[0]?.league?.standings?.[0]?.[0]) {
                    ...
                    result.statusLabel = `👑 ${champion.name}`;
                }
            }
    ...
    async getTeamsWithStandings(leagueId, season, country = null) {
        ...
        try {
            ...
            const mergedTeams = teamsData.map(item => {
                ...
                // 2. If no fixtures but have standings -> team must be in standings (and check knockout rules)
                ...
                if (activeTeamIds.size > 0) {
                    ...
                    // 🔴 Apply knockout rules from tournament configuration
                    if (isActive && standing) {
                        const knockoutRules = getTournamentKnockoutRules(leagueId);
                        if (knockoutRules && standing.rank > knockoutRules.qualificationCutoff) {
                            isActive = false;
                            console.log(`[API] Team ${team.name} eliminated (Rank ${standing.rank} > ${knockoutRules.qualificationCutoff})`);
                        }
                    }
                }
            ...
            console.log(`[API] Filtered from ${mergedTeams.length} to ${filteredTeams.length} active teams (method: ${useStandingsForFiltering ? 'standings+rank' : activeTeamIds.size > 0 ? 'fixtures' : 'show-all'})`);
